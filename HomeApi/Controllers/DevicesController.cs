...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Delete(
            [FromRoute] Guid id)
        {
            var item = await _devices.GetDeviceById(id);
            if (item == null)
            {
                return StatusCode(400, $"Ошибка: Устройство с идентификатором {id} не существует.");
            };
            await _devices.DeleteDevice(item);
            return StatusCode(200, $"Устройство удалено! Имя - {item.Name}, Серийный номер - {item.SerialNumber},  Комната подключения - {item.Room.Name}");
        }
        /// <summary>
